...
### Step 2: Start local server and open Chrome preview
Start a Python HTTP server from the LECWeb root and open the course index in a narrow incognito Chrome window:

```bash
# Start local server on port 8111 (if not already running)
lsof -ti:8111 >/dev/null 2>&1 || python3 -m http.server 8111 --directory /Users/sergioparreiras/Dropbox/Teaching/Projects/LECWeb &>/dev/null &

# Open in narrow incognito Chrome
"/Applications/Google Chrome.app/Contents/MacOS/Google Chrome" --incognito --window-size=500,1080 "http://localhost:8111/[course]/index.html" &>/dev/null &
```

Replace `[course]` with `101` or `510` based on Step 1. The narrow width (500px) minimizes screen footprint so the user can keep their editor alongside. Port 8111 is used to avoid conflicts with common dev servers.

**Why localhost, not file://:** `file://` URLs break CORS, module loading, and the Chrome extension (`mcp__claude-in-chrome__navigate`) prepends `https://` to them. A local server avoids all these issues and lets the Chrome extension navigate pages normally.
